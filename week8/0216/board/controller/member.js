...
exports.login = async (req, res) => {
    ...
    if (result) {
        ...
        if (save === 'save') {
            res.cookie(SAVEID, result.id, { maxAge: 30 * 24 * 60 * 60 * 1000, httpOnly: true });
        } else {
...
exports.getCookie = (req, res) => {
    const result = Member.findOne({ where: { id: this.getCookie('saveId') } });
    req.session.member = result;
    res.json({ result, login: req.cookies });
};
